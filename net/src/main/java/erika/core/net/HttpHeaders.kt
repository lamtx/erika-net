...
@JvmInline
value class HttpHeaders(private val headers: MutableMap<String, MutableList<String>>) {

    operator fun get(key: String): String? {
        return headers[key.lowercase()]?.last()
    }

    val contentType: String? get() = this[ContentType]

    val contentLength: Long
        get() = this[ContentLength]?.toLongOrNull() ?: -1L

    val contentLengthInt: Int
        get() = this[ContentLength]?.toIntOrNull() ?: -1
